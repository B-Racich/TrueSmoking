if getActivatedMods().contains('EvolvingTraitsWorld') then
    ...
    function ETWOnEat(character, percent)
        if isClient() or (not isClient() and not isServer()) then
            ...
            local addictionGain = SBvars.SmokingAddictionMultiplier * (1 + stress) * (1 + panic / 100) * 1000 / (math.max(timeSinceLastSmoke, smokerModData.MinutesSinceLastSmoke) + 100) * percent;
